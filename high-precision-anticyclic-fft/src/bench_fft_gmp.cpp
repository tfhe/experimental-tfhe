...
struct Real96 {
    ...
    Real96() { mpz_init(v); }
    Real96(const Real96& r) { mpz_init_set(v,r.v); }
    void operator=(const Real96& r) { mpz_set(v,r.v); }
    ~Real96() { mpz_clear(v); }
};

void add(Real96& dest, const Real96& a, const Real96& b) {
    mpz_add(dest.v,a.v,b.v);
}
void sub(Real96& dest, const Real96& a, const Real96& b) {
    mpz_sub(dest.v,a.v,b.v);
}
void neg(Real96& dest, const Real96& a) {
    mpz_neg(dest.v,a.v);
}
void extmul(Real96& dest, int a, const Real96& b) {
    mpz_mul_si(dest.v,b.v,a);
}
void intmul(Real96& dest, const Real96& a, const Real96& b) {
    mpz_mul(dest.v,a.v,b.v);
    mpz_tdiv_q_2exp(dest.v,dest.v,FPREC);
    mpz_tdiv_r_2exp(dest.v,dest.v,FPREC+IPREC);
}

void div2ui(Real96& dest, const Real96& a, uint64_t b) {
    mpz_tdiv_q_2exp(dest.v,a.v,b);
}

Real96 t64tor96(Torus64 v) {
    Real96 reps; 
    int64_t vv = v;
    mpz_set_si(reps.v, vv);
    mpz_mul_2exp(reps.v, reps.v, FPREC-64);
    return reps;
}

Real96 dtor96(double d) {
    Real96 reps; 
    mpz_set_d(reps.v, d*pow(2.,FPREC));
    return reps;
}
...
double r96tod(const Real96& r) {
    return mpz_get_d(r.v) / pow(2.,FPREC);
}
...
Real96 accurate_cos(int i,int n) { //cos(2pi*i/n)
    ...
    mpfr_mul_2si(res, res, FPREC,MPFR_RNDN);
    mpfr_get_z(reps.v,res,MPFR_RNDN);
    return reps;
}
